"""vLLM backend for the `cot`, `cot_greedy` and `soft` methods.

`selar` and `swir` are HF-only: they rewrite the running sequence mid-flight and read
per-step entropy trends that the vLLM engine does not expose. `scripts/run.py --backend
vllm` rejects them rather than quietly running a different algorithm.
...
SUPPORTED_METHODS = ("cot", "cot_greedy", "soft")
...
def check_method_supported(method):
    ...
    if method not in SUPPORTED_METHODS:
        raise ValueError(
            f"--backend vllm supports {' and '.join(SUPPORTED_METHODS)}, not '{method}'. "
            f"selar and swir need per-step control of the decode loop that the vLLM "
            f"engine does not expose; run them with --backend hf."
        )
...
def build_llm(model_name, *, tensor_parallel_size=1, gpu_memory_utilization=0.90,
              ...
              soft_thinking=False, reasoning_parser="qwen3"):
    """Construct the engine. Kept separate so the import cost lands only on this path."""
    if soft_thinking:
        # Soft Thinking is implemented in vLLM's V1 GPU model runner. Dense
        # non-MoE models (all of Qwen3) default to the V2 runner, which would
        # silently decode plain CoT -- the engine refuses soft requests under
        # V2, and this pin is how the refusal never fires from run.py. Hybrid
        # models (Qwen3.5) select V1 regardless, so for them this is a no-op.
        # Must be set before the engine builds: worker processes inherit it.
        import os
    ...
    if soft_thinking:
        # All three are refused by the engine if missing, so set them here rather
        # than make every caller remember: the reasoning parser is what resolves
        # </think>, the concept token is fed back through inputs_embeds, and the
        # prefix cache is keyed by token ids that a concept token's KV did not
        # come from.
